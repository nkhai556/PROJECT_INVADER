...
public class E_Summoner extends Enemy implements Villians {
    ...
    public E_Summoner(int x, int y, int limit, Player player) {
        super.x = x;
        super.y = y;
        super.HP = 5;
    ...
    public void addBullet (int Px, int Py){
        bullets.add(new EB_Homing(x + +width + width/2, y + getTilesize()/2, player));
        bullets.add(new EB_Homing(x - width/2, y + getTilesize()/2, player));
    }
}
